...
class StackPlugin(ROSPlugin):
    ...
    async def list_stacks(self, stack_id=None, stack_name=None):
        request_kwargs = dict(StackId=stack_id, StackName=[stack_name] if stack_name else None)
    ...
    async def get_stack_resource(
        self, stack_id, logical_resource_id, show_resource_attributes=False, resource_attributes=None
    ):
        kwargs = dict(StackId=stack_id, LogicalResourceId=logical_resource_id)
        if show_resource_attributes:
            kwargs.update(ShowResourceAttributes='true')
    ...
    async def get_parameter_constraints(
        ...
        parameters_key_filter: list = None,
        parameters: dict = None,
        parameters_order: list = None,
    ...
    async def get_template(self, template_id: str, template_version: str = None):
        kwargs = dict(TemplateId=template_id, TemplateVersion=template_version)
        return await self.send_request('GetTemplate', **kwargs)

    async def get_template_estimate_cost(
        self, template_body: str = None, parameters: dict = None, region_id: str = None, template_url: str = None
    ):
        kwargs = dict(TemplateBody=template_body, TemplateURL=template_url, RegionId=region_id)
        self._convert_parameters(parameters, kwargs)
        result = await self.send_request('GetTemplateEstimateCostRequest', **kwargs)
        return result['Resources']

    async def validate_template(self, template_body: str = None, region_id: str = None, template_url: str = None):
        kwargs = dict(TemplateBody=template_body, TemplateURL=template_url, RegionId=region_id)
    ...
    async def preview_stack(
        ...
        kwargs = dict(TemplateBody=template_body, TemplateURL=template_url, RegionId=region_id, StackName=stack_name)
        self._convert_parameters(parameters, kwargs)
    ...
    async def generate_template_policy(self, template_body: str = None, template_url: str = None):
        kwargs = dict(TemplateBody=template_body, TemplateURL=template_url)
