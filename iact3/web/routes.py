...
async def _resolve_terraform_params(tests_params, region, credential, multi_region=False):
    ...
    if not multi_region:
        # Third pass: resolve ECS-specific params that depend on zone_id
        for key, val in resolved.items():
            if not isinstance(val, str) or not RE_V_AUTO.fullmatch(val):
                continue
            if RE_K_INSTANCE_TYPE.fullmatch(key):
                try:
                    ecs_plugin = EcsPlugin(region, credential=credential)
                    types = await ecs_plugin.describe_available_instance_types(zone_id=zone_id)
                    if types:
                        resolved[key] = pick_cheapest_instance_type(types)
                except Exception:
                    pass
            elif RE_K_SYSTEM_DISK.fullmatch(key):
                resolved[key] = 'cloud_essd'
...
def setup_routes(app: web.Application):
    ...
    async def start_run(request):
        ...
        # Persist original editor contents so the run detail page can display them.
        # Use a raw_* prefix so StackTest.from_file does not pick these up.
                ...
                if tpl_body:
                    ...
                    if isinstance(tpl_parsed, dict):
                        ...
                        if isinstance(tpl_params, dict):
                            ...
                            # Priority: 1. Default value from template, 2. $[iact3-auto]
                            for param_name, param_def in tpl_params.items():
                                if param_name not in tests_params:
